export const paging = (current, This) => {
    ...
    if (isNaN(current)) return This.state.dataPerPage;
    This.setState({ paginate: {current: current, perPage } });
    if (current > Math.ceil(data.length / perPage)) {
        This.setState({ paginate: {current: current - 1 , perPage} });
        return dataPerPage;
    }
    if (current <= 1 || isNaN(current)) {
        current = 1;
        This.setState({ paginate: {current: 1, perPage } });
        return data.slice((current - 1) * perPage, (current) * perPage);
    }
    This.setState({ paginate: {current, perPage} });
